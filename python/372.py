# 你的任务是计算 a 的 b 次方 然后对 1337 取模，a 是一个正整数，b 是一个非常大的正整数且会以数组形式给出。
...
class Solution:
    def superPow(self, a: int, b: [int]) -> int:
        print('n = %d' % self.phiwheel(1137))
        sum = 0
        for n in b:
            sum = sum * 10 + n
        
        return pow(a, sum % 1140, 1337) 
...
# print(Solution().superPow(2, [3,3]))
start = time.clock()
#long running
#do something other
# # print(Solution().superPow(2, [3]))
# print(Solution().superPow(2, [1, 0]))
# print(Solution().superPow(2, [3,3,3,3,3,3,3,3,3,3,3]))
